...
import { medicalRecordAPI, MedicalRecord, VitalSigns, Allergy, MedicalCondition } from '../../../../services/medicalRecordAPI';
...
export default function PatientMedicalRecords() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'vitals' | 'allergies' | 'conditions' | 'medications'>('overview');
  ...
  const fetchData = async () => {
    try {
      ...
      const recordResult = await medicalRecordAPI.getPatientMedicalRecord(patientId as string);
  ...
  const handleAddVitalSigns = async () => {
    try {
      await medicalRecordAPI.addVitalSigns(patientId as string, vitalSigns as VitalSigns);
  ...
  const handleEditVitalSigns = async () => {
    try {
      ...
      await medicalRecordAPI.updateVitalSigns(editingVital._id, vitalSignsToUpdate, patientId as string);
  ...
  const handleDeleteVitalSigns = (vital: VitalSigns) => {
    Alert.alert(
      'Delete Vital Signs',
      `Are you sure you want to delete the vital signs from ${formatDate(vital.createdAt || '')}?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              if (!vital._id) return;
              await medicalRecordAPI.removeVitalSigns(vital._id, patientId as string);
              await fetchData(); // Refresh data
              Alert.alert('Success', 'Vital signs deleted successfully');
            } catch (error: any) {
              Alert.alert('Error', error.message || 'Failed to delete vital signs');
            }
          }
        }
      ]
  ...
  const renderVitalSignsForm = () => {
    return (
      <View style={{ gap: 16 }}>
        <View>
          <Text style={{ fontSize: 16, fontWeight: '600', marginBottom: 8 }}>Blood Pressure</Text>
          <View style={{ flexDirection: 'row', gap: 12 }}>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 14, color: '#6B7280', marginBottom: 4 }}>Systolic</Text>
              <TextInput
                ...
                onChangeText={(text) => setVitalSigns(prev => ({
                  ...prev,
                  bloodPressure: { ...prev.bloodPressure!, systolic: parseInt(text) || 0 }
                }))}
                placeholder="120"
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 14, color: '#6B7280', marginBottom: 4 }}>Diastolic</Text>
              <TextInput
                style={{ borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, padding: 12, fontSize: 16 }}
                value={vitalSigns.bloodPressure?.diastolic?.toString() || ''}
                onChangeText={(text) => setVitalSigns(prev => ({
                  ...prev,
                  bloodPressure: { ...prev.bloodPressure!, diastolic: parseInt(text) || 0 }
                }))}
                placeholder="80"
                keyboardType="numeric"
              />
        ...
        <View>
          <Text style={{ fontSize: 16, fontWeight: '600', marginBottom: 8 }}>Heart Rate (bpm)</Text>
          <TextInput
            style={{ borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, padding: 12, fontSize: 16 }}
            value={vitalSigns.heartRate?.toString() || ''}
            onChangeText={(text) => setVitalSigns(prev => ({ ...prev, heartRate: parseInt(text) || 0 }))}
            placeholder="70"
            keyboardType="numeric"
          />
        </View>
        ...
        <View>
          <Text style={{ fontSize: 16, fontWeight: '600', marginBottom: 8 }}>Temperature (°F)</Text>
          <TextInput
            style={{ borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, padding: 12, fontSize: 16 }}
            value={vitalSigns.temperature?.toString() || ''}
            onChangeText={(text) => setVitalSigns(prev => ({ ...prev, temperature: parseFloat(text) || 0 }))}
            placeholder="98.6"
        ...
        <View>
          <Text style={{ fontSize: 16, fontWeight: '600', marginBottom: 8 }}>Weight (lbs)</Text>
          <TextInput
            style={{ borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, padding: 12, fontSize: 16 }}
            value={vitalSigns.weight?.toString() || ''}
            onChangeText={(text) => setVitalSigns(prev => ({ ...prev, weight: parseFloat(text) || 0 }))}
            placeholder="150"
            keyboardType="decimal-pad"
          />
        </View>
        ...
        <View>
          <Text style={{ fontSize: 16, fontWeight: '600', marginBottom: 8 }}>Height (inches)</Text>
          <TextInput
            style={{ borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, padding: 12, fontSize: 16 }}
            value={vitalSigns.height?.toString() || ''}
            onChangeText={(text) => setVitalSigns(prev => ({ ...prev, height: parseFloat(text) || 0 }))}
            placeholder="68"
            keyboardType="decimal-pad"
          />
        </View>
        ...
        <View>
          <Text style={{ fontSize: 16, fontWeight: '600', marginBottom: 8 }}>Oxygen Saturation (%)</Text>
          <TextInput
            style={{ borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, padding: 12, fontSize: 16 }}
            value={vitalSigns.oxygenSaturation?.toString() || ''}
            onChangeText={(text) => setVitalSigns(prev => ({ ...prev, oxygenSaturation: parseInt(text) || 0 }))}
            placeholder="98"
            keyboardType="numeric"
          />
        </View>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f5f5f5' }}>
      {/* Header */}
      <View style={{ backgroundColor: '#fff', paddingHorizontal: 16, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#E5E7EB' }}>
        <TouchableOpacity
          onPress={() => router.back()}
          ...
          <View>
            ...
            {medicalRecord?.vitalSigns && medicalRecord.vitalSigns.length > 0 && (
              <View style={{ backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 16 }}>
                ...
                {(() => {
                  const latest = medicalRecord.vitalSigns[medicalRecord.vitalSigns.length - 1];
                  return (
                    <View style={{ gap: 8 }}>
                      {latest.bloodPressure && (
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                          <Text style={{ color: '#6B7280' }}>Blood Pressure:</Text>
                          <Text style={{ color: '#374151', fontWeight: '500' }}>
                      ...
                      {latest.heartRate && (
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                          <Text style={{ color: '#6B7280' }}>Heart Rate:</Text>
                          <Text style={{ color: '#374151', fontWeight: '500' }}>
                      ...
                      {latest.temperature && (
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                          <Text style={{ color: '#6B7280' }}>Temperature:</Text>
                          <Text style={{ color: '#374151', fontWeight: '500' }}>
          ...
          <View>
            ...
            {medicalRecord?.vitalSigns && medicalRecord.vitalSigns.length > 0 ? (
              [...medicalRecord.vitalSigns].reverse().map((vital, index) => (
                <View key={vital._id || index} style={{ backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 12 }}>
                  ...
                  <View style={{ gap: 6 }}>
                    ...
                    {vital.heartRate && (
                      <Text style={{ color: '#374151' }}>Heart Rate: {vital.heartRate} bpm</Text>
                    )}
                    {vital.temperature && (
                      <Text style={{ color: '#374151' }}>Temperature: {vital.temperature}°F</Text>
                    )}
                    {vital.weight && (
                      <Text style={{ color: '#374151' }}>Weight: {vital.weight} lbs</Text>
                    )}
                    {vital.height && (
                      <Text style={{ color: '#374151' }}>Height: {vital.height} in</Text>
                    )}
                    {vital.oxygenSaturation && (
                      <Text style={{ color: '#374151' }}>Oxygen Saturation: {vital.oxygenSaturation}%</Text>
                    )}
                  </View>
                </View>
              ))
          ...
          <View>
            ...
            {medicalRecord?.allergies && medicalRecord.allergies.length > 0 ? (
              medicalRecord.allergies.map((allergy, index) => (
                <View key={index} style={{ backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 12 }}>
                  ...
                  <View
                    style={{
                      backgroundColor: allergy.severity === 'severe' || allergy.severity === 'critical' ? '#FEF2F2' : '#FEF3C7',
                      paddingHorizontal: 8,
                      paddingVertical: 4,
                      borderRadius: 6,
                  ...
                  >
                    <Text
                      style={{
                        color: allergy.severity === 'severe' || allergy.severity === 'critical' ? '#DC2626' : '#D97706',
                        fontSize: 12,
                        fontWeight: '600',
            ...
            {medicalRecord?.medicalConditions && medicalRecord.medicalConditions.length > 0 ? (
              medicalRecord.medicalConditions.map((condition, index) => (
                <View key={index} style={{ backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 12 }}>
                  ...
                  <View
                    style={{
                      backgroundColor: condition.status === 'active' ? '#FEF2F2' : condition.status === 'chronic' ? '#FEF3C7' : '#F0FDF4',
                      paddingHorizontal: 8,
                      paddingVertical: 4,
                      borderRadius: 6,
                      alignSelf: 'flex-start',
                  ...
                  >
                    <Text
                      style={{
                        color: condition.status === 'active' ? '#DC2626' : condition.status === 'chronic' ? '#D97706' : '#059669',
                        fontSize: 12,
                        fontWeight: '600',
                        textTransform: 'capitalize',
                      }}
                  ...
                  {condition.notes && (
                    <Text style={{ color: '#6B7280', marginTop: 8, fontStyle: 'italic' }}>
      ...
      >
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: '#E5E7EB' }}>
            ...
            <Text style={{ fontSize: 18, fontWeight: 'bold' }}>Add Vital Signs</Text>
      ...
      >
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: '#E5E7EB' }}>
            ...
            <Text style={{ fontSize: 18, fontWeight: 'bold' }}>Edit Vital Signs</Text>
            <TouchableOpacity onPress={handleEditVitalSigns}>
              <Text style={{ color: '#007AFF', fontSize: 16, fontWeight: '600' }}>Update</Text>
            </TouchableOpacity>
          </View>
