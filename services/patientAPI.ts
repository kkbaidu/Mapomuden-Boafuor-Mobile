...
class PatientAPI {
  ...
  async getDoctorPatients(filters: PatientFilters = {}) {
    try {
      ...
      let patients = allPatients.map(patient => {
      ...
      if (filters.location) {
        patients = patients.filter(p => 
          p.location && filters.location && p.location.toLowerCase().includes(filters.location.toLowerCase())
        );
      }
  ...
  async getPatientDetails(patientId: string) {
    try {
      ...
      const appointments = appointmentsResponse.data.appointments?.filter(
        (apt: any) => {
          const aptPatientId = typeof apt.patient === 'object' ? apt.patient._id : apt.patient;
          return aptPatientId === patientId;
        }
      ...
      const lastVisitAppointment = completedAppointments
        .sort((a: any, b: any) => new Date(b.appointmentDate).getTime() - new Date(a.appointmentDate).getTime())[0];
